...
namespace MidiPlayerTK
{
    ...
    public class SoundFontSetupWindow : EditorWindow
    {
        ...
        static PopupList popupLoadType;
        static PopupList popupCompressionFormat;
        ...
        private void ShowListSoundFonts(float startX, float startY, float width, float height)
        {
            try
            {
                ...
                {
                    GUI.Label(new Rect(boxX + column.PositionCaption, startY + localstartY, column.Width, itemHeight), column.Caption, MidiCommonEditor.styleLabelLeft);
                    boxX += column.Width;
                }
        ...
        {
            string infocheck = this.CheckAndSetBank(LoadType, CompressionFormat);
        ...
        {
            if (popupLoadType == null)
            {
                //Debug.Log("BuildPopup popupLoadType");
                List<string> listLoadType = new List<string>();
                foreach (string loadType in Enum.GetNames(typeof(AudioClipLoadType)))
                    listLoadType.Add(loadType);
                popupLoadType = new PopupList("Audio Load Type", true, listLoadType);
                popupLoadType.OnSelect = LoadTypeChanged;
                LoadType = popupLoadType.SelectedIndex = MidiPlayerGlobal.ImSFCurrent.LoadType;

            }
            if (popupCompressionFormat == null)
            {
                //Debug.Log("BuildPopup popupCompressionFormat");
                List<string> listCompressionFormat = new List<string>();
                foreach (string loadType in Enum.GetNames(typeof(AudioCompressionFormat)))
                    listCompressionFormat.Add(loadType);
                popupCompressionFormat = new PopupList("Audio Compression Format", true, listCompressionFormat);
                popupCompressionFormat.OnSelect = CompressionFormatChanged;
                CompressionFormat = popupCompressionFormat.SelectedIndex = MidiPlayerGlobal.ImSFCurrent.CompressionFormat;
            }
        }
        private void LoadTypeChanged(int index)
        {
            Debug.Log("Selected Load Type: " + (AudioClipLoadType)index);
            LoadType = index;
        }
        private void CompressionFormatChanged(int index)
        {
            Debug.Log("Selected Compression Format: " + (AudioCompressionFormat)index);
            CompressionFormat = index;
        ...
        private void ShowExtractOptim(float localstartX, float localstartY, float width, float height)
        {
            try
            {
                ...
                if (MidiPlayerGlobal.ImSFCurrent != null)
                {
                    ...
                    EditorGUI.LabelField(new Rect(xpos, ypos, 380 + 85, itemHeight), content, MidiCommonEditor.styleBold);
                    ypos += itemHeight;// + espace;

                    content = new GUIContent() { text = "Settings:", tooltip = "" };
                    EditorGUI.LabelField(new Rect(xpos, ypos, 380 + 85, itemHeight), content, MidiCommonEditor.styleBold);
                    ypos += itemHeight;
                    ...
                    */
                    RemoveUnusedWaves = GUI.Toggle(new Rect(xpos, ypos, widthCheck, itemHeight), RemoveUnusedWaves,
                    ...
                    Rect btrect = new Rect(xpos + widthCheck + espace, ypos, 300, itemHeight);
                    if (GUI.Button(btrect, new GUIContent($"Audio Load Type: {popupLoadType.SelectedLabel}", "")))
                        PopupWindow.Show(btrect, popupLoadType);

                    ypos += lineHeight;

                    LogDetailSoundFont = GUI.Toggle(new Rect(xpos, ypos, widthCheck, itemHeight), LogDetailSoundFont,
                        new GUIContent("Log SoundFont Detail", "If check, keep only waves used by your midi files"), MidiCommonEditor.styleToggle);

                    if (popupCompressionFormat == null)
                        BuildPopup();
                    btrect = new Rect(xpos + widthCheck + espace, ypos, 300, itemHeight);
                    if (GUI.Button(btrect, new GUIContent($"Audio Compression Format: {popupCompressionFormat.SelectedLabel}", "")))
                        PopupWindow.Show(btrect, popupCompressionFormat);
                    ...
                    {
                        ypos += (lineHeight * 3) / 4;
                        content = new GUIContent() { text = "You need to extract to apply.", tooltip = "" };
                        EditorGUI.LabelField(new Rect(xpos, ypos, 380 + 85, itemHeight), content, MidiCommonEditor.styleAlertRed);
                    }

                    ypos += lineHeight;
                    content = new GUIContent() { text = "Extract Patches and Samples from the SoundFont:", tooltip = "" };
                    EditorGUI.LabelField(new Rect(xpos, ypos, 380 + 85, itemHeight), content, MidiCommonEditor.styleBold);
                    ypos += itemHeight;
                    Rect rect1 = new Rect(xpos, ypos, 210, (float)buttonHeight * 2f);
                    Rect rect2 = new Rect(xpos + 210 + 3, ypos, 210, (float)buttonHeight * 2f);
#if MPTK_PRO
                    if (GUI.Button(rect1, new GUIContent("Extract Only Required", "Your list of Midi files will be scanned to identify patches and samples needed")))
                    {
                        if (Application.isPlaying)
                    ...
                    if (GUI.Button(rect2, new GUIContent("Extract All", "Extract all patchs and waves from the Soundfont for the selected banks")))
                    {
                        if (Application.isPlaying)
                ...
                else
                {
                    ...
                    EditorGUI.LabelField(new Rect(localstartX + xpostitlebox, localstartY + ypostitlebox, 300, itemHeight), content, MidiCommonEditor.styleBold);
                }
            }
            catch (ExitGUIException) { }
            catch (Exception ex)
            ...
            try
            {
                ...
                if (countLine > 0)
                {
                    ...
                    if (displayAlert)
                    {
                        ...
                        ypos += 2 * fHeight;
                        EditorGUI.LabelField(new Rect(xpos, ypos, fwidth, fHeight), "On the right panel:", MidiCommonEditor.styleAlertRed);
                        ypos += fHeight;
                        EditorGUI.LabelField(new Rect(xpos, ypos, fwidth, fHeight), "   Select banks you want to keep.", MidiCommonEditor.styleAlertRed);
                        ypos += fHeight;
                        EditorGUI.LabelField(new Rect(xpos, ypos, fwidth, fHeight), "   Select default bank for instruments and drums kit.", MidiCommonEditor.styleAlertRed);
                        ypos += 2 * fHeight;
                        EditorGUI.LabelField(new Rect(xpos, ypos, fwidth, fHeight), "Click on buttons:", MidiCommonEditor.styleAlertRed);
                        ypos += fHeight;
                        EditorGUI.LabelField(new Rect(xpos, ypos, fwidth, fHeight), "   'Optimize from Midi file list' to keep only required patchs.", MidiCommonEditor.styleAlertRed);
                        ypos += fHeight;
                        EditorGUI.LabelField(new Rect(xpos, ypos, fwidth, fHeight), "      or", MidiCommonEditor.styleAlertRed);
                        ypos += fHeight;
                        EditorGUI.LabelField(new Rect(xpos, ypos, fwidth, fHeight), "   'Extract all patchs & samples' to keep all from selected banks.", MidiCommonEditor.styleAlertRed);
                        ypos += fHeight;
